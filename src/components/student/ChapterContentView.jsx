...
function AnswerKeyReview({ quizData }) {
...
export default function ChapterContentView({ chapter, onQuizComplete }) {
  ...
  if ((interactiveType === 'answer-key' || isAnswerKeyChapter(chapter)) && quizData?.questions?.length) {
    return <AnswerKeyReview quizData={quizData} />
  }
  ...
  if (html) {
    return (
      <div
        className="study-notes-html mt-4 max-w-none rounded-xl border border-gray-700 bg-gray-900/80 p-4 text-sm leading-relaxed text-gray-200
          [&_h1]:text-xl [&_h1]:font-bold [&_h2]:mt-4 [&_h2]:text-lg [&_h2]:font-semibold [&_h3]:mt-2 [&_h3]:font-semibold
          [&_ul]:my-2 [&_ul]:list-disc [&_ul]:pl-5 [&_ol]:my-2 [&_ol]:list-decimal [&_ol]:pl-5
          [&_table]:my-3 [&_table]:w-full [&_th]:bg-gray-800 [&_th]:p-2 [&_td]:border [&_td]:border-gray-700 [&_td]:p-2
          [&_a]:text-violet-300"
